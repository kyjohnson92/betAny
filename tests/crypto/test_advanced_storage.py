...
def adv_storage_contract(w3, get_contract):
    with open('src/crypto/advanced_storage.vy') as f:
...
def test_initial_state(adv_storage_contract):
    ...
    assert adv_storage_contract.stored_data() == INITIAL_VALUE
...
def test_failed_transactions(w3, adv_storage_contract, assert_tx_failed):
    ...
    assert_tx_failed(
        lambda: adv_storage_contract.set(-10, transact={"from": k1}))
    ...
    adv_storage_contract.reset(transact={"from": k0})
    adv_storage_contract.set(11, transact={"from": k1})
    assert adv_storage_contract.stored_data() == 11
